...
describe("online room service", () => {
  ...
  it("starts the next game with randomized opposite-seat teams while retaining appearances and match history", async () => {
    const store = new InMemoryRoomStore();
    let token = 0;
    const service = new RoomService(store, {
      createRoomId: () => "1234",
    ...
    expect(next.room.session.game.dealer).toBe("P1");
    ...
    expect(next.room.matchHistory[0]).toEqual(expect.objectContaining({
      ...
      teamParticipantIds: [["player-P1", "player-P3"], ["player-P2", "player-P4"]],
    }));
    expect(next.room.matchHistory[0].players.every((player) => player.timesEliminated === 0)).toBe(true);
    expect(next.room.playerNames.P2).toBe("Ian");
    expect(next.room.playerNames.P4).toBe("Sunny");
    expect(next.room.playerNames.P1).toBe("Omi");
    expect(next.room.playerNames.P3).toBe("Fern");
    expect(next.room.appearanceSeats).toEqual({ P1: "P2", P2: "P1", P3: "P4", P4: "P3" });
    const nextStored = await store.get("1234");
    expect(nextStored?.room.participantIds?.P2).toBe("player-P1");
    expect(nextStored?.room.participantIds?.P4).toBe("player-P3");
    expect(nextStored?.room.participantIds?.P1).toBe("player-P2");
    expect(nextStored?.room.participantIds?.P3).toBe("player-P4");
  });
  ...
  it("allows a genuinely randomized rematch to keep the same positions", async () => {
    const store = new InMemoryRoomStore();
    let token = 0;
    const service = new RoomService(store, {
      createRoomId: () => "1234",
    ...
    const next = await service.startNextGame("1234", host.access.playerToken, { randomizeSeats: true });

    expect(next.access.playerId).toBe("P1");
    expect(next.room.playerNames).toMatchObject({ P1: "Ian", P2: "Omi" });
    expect(next.room.appearanceSeats).toEqual({ P1: "P1", P2: "P2" });
  });
